...
static func corp_install_cost(state: NRState, side: Variant, card: Dictionary, server: Variant, args: Dictionary = {}) -> Array:
	...
	if base == null:
		base = []
	var ignore_ice = NRUtil.truthy(args.get("ignore-ice-cost", false))
	...
	if not NRUtil.truthy(args.get("ignore-install-cost", false)):
...
static func corp_install(state: NRState, side: Variant, eid: Dictionary, card: Dictionary, server: Variant, args: Dictionary = {}) -> void:
	...
	, func(payment):
		...
		var name = moved.get("title") if moved is Dictionary and (NRCard.ice(moved) == false or NRUtil.truthy(args.get("known"))) else ("ice" if NRCard.ice(c) else "a card")
		if NRCard.ice(c):
			NRSay.system_msg(state, "corp", "%sice protecting %s" % [NRPayment.build_spend_msg(cost_str, "install"), NRServers.zone_to_name(dest)])
		else:
			NRSay.system_msg(state, "corp", "%sa card in the root of %s" % [NRPayment.build_spend_msg(cost_str, "install"), NRServers.zone_to_name(dest)])
...
static func runner_can_install(state: NRState, _side: Variant, card: Dictionary, args: Dictionary = {}) -> bool:
	if install_locked(state, "runner"):
		...
		return false
	if NRCard.program(card) and not NRMemory.sufficient_mu(state, card) and not NRUtil.truthy(args.get("ignore-mu", false)):
		if not NRUtil.truthy(args.get("no-toast", false)):
...
static func runner_install_cost(state: NRState, side: Variant, card: Dictionary, args: Dictionary = {}) -> Array:
	...
	if not NRUtil.truthy(args.get("ignore-install-cost", false)) and not NRCostFns.ignore_install_cost(state, side, card):
...
static func runner_install(state: NRState, side: Variant, eid: Dictionary, card: Dictionary, args: Dictionary = {}) -> void:
	...
	, func(payment):
		if not (payment is Dictionary) or payment.get("cost-paid") == null:
			NREid.effect_completed(state, side, eid)
			return
		var dest = ["rig", "facedown"] if NRUtil.truthy(args.get("facedown", false)) else NRServers.type_to_rig_zone(str(c.get("type")))
		var moved = NRMoving.move(state, "runner", c, dest, args)
		if moved is Dictionary:
			NRInitializing.card_init(state, "runner", moved, {"resolve-effect": true, "init-data": true, "no-mu": NRUtil.truthy(args.get("no-mu", false))})
		var cost_str = str(payment.get("msg", ""))
...
static func corp_install_msg(card: Dictionary) -> String:
	return "install %s from %s" % [("an unseen card" if not NRUtil.truthy(card.get("seen")) else card.get("title")), NRServers.name_zone("corp", card.get("zone"))]
