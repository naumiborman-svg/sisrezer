class_name ClojureClient
extends Node

var base_url := "http://127.0.0.1:1042"
var last_error := ""
...
func get_state(id: String) -> Dictionary:
	return await _req(HTTPClient.METHOD_GET, "/godot/state?id=%s" % id.uri_encode())


func action(id: String, act: Dictionary) -> Dictionary:
	var body: Dictionary = {
		"id": id,
		"command": str(act.get("command", act.get("op", ""))),
		"side": str(act.get("side", "")),
		"args": act.get("args", {}),
	}
	return await _req(HTTPClient.METHOD_POST, "/godot/action", body)
...
func _req(method: int, path: String, payload: Variant = null) -> Dictionary:
	...
	if parsed is Dictionary:
		...
		if code >= 400 and not d.has("error"):
			d["error"] = "HTTP %s" % code
		if code >= 400:
			d["ok"] = false
		return d
